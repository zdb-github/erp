	...
	$('#updateGoods').click(function () {
                ...
                $( "#alert-win" ).dialog({
				   close: function(event, ui) {
				    ...
				    window.location.href="sale/toUpdateSale.do?sale_id=${erpSale.sale_id}";
				   }
				});
            })
	
   	$("#backToList").click(function(){
		window.location.href="sale/toListSale.do";
	...
	function jz(){
		...
		var supplierArr = document.getElementById("customer_id1");
	  		for(var i=0;i<supplierArr.options.length;i++){
	  			if(supplierArr.options[i].value=="${erpSale.customer_id}"){
	  		...
	  		for(var i=0;i<deptArr.options.length;i++){
	  			if(deptArr.options[i].value=="${erpSale.dept_id}"){
	  		...
	  		for(var i=0;i<organizationArr.options.length;i++){
	  			if(organizationArr.options[i].value=="${erpSale.organization_id}"){
	  		...
	  		for(var i=0;i<purchaserArr.options.length;i++){
	  			if(purchaserArr.options[i].value=="${erpSale.salesman_id}"){
	  		...
	  		for(var i=0;i<stateArr.options.length;i++){
	  			if(stateArr.options[i].value=="${erpSale.invoices_state}"){
	  		...
	  		for(var i=0;i<deliveryArr.options.length;i++){
	  			if(deliveryArr.options[i].value=="${erpSale.delivery_way}"){
	  		...
	  		for(var i=0;i<warehouseArr.options.length;i++){
	  			if(warehouseArr.options[i].value=="${erpSale.payment_method}"){
	  		...
	  		for(var i=0;i<saleTypeArr.options.length;i++){
	  			if(saleTypeArr.options[i].value=="${erpSale.sale_type}"){
...
function selectAll() {
	...
	<% int count = 1; %>
		...
		<div id="po">
		<form action="sale/updateSale.do" method="post" id="myForm1"> 
			<table class="table table-striped table-bordered table-condensed">
				<thead>
					<tr>
						<td colspan="6" class="auto-style2">
							&nbsp;请修改销售单${erpSale.sale_id}的内容
						</td>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<td >
							<input id="sale_id1" name="sale_id" style="width:220px; height:30px"  type="text" value="${erpSale.sale_id}" readonly="readonly" />
						</td>
						<td>
							单据日期
						</td>
						<td>
							<input id="date1" class="span2 datepicker" style="width:220px; height:30px" type="text" name="create_time" value="${erpSale.create_time}" />
	...
	<div id="alert-win" title="修改销售单商品" style="display: none;">
        <iframe style="border: 0; width: 100%; height: 100%;" src="sale/toListSaleGoods.do?sale_id=${erpSale.sale_id}"></iframe>
    </div>
