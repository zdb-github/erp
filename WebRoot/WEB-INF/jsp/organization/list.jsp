...
$(document).ready(function() {
	...
	$(".cancel").click(function() {
		...
		<script type="text/javascript">
function toAdd() {
	window.location.href = "<%=basePath%>organization/toAdd.do";
}
function toList() {
	window.location.href = "<%=basePath%>organization/toList.do";
}

function update(num) {
	document.getElementById("myFormUpdate" + num).submit();
}
function deleteOrgan(num) {
	if (confirm("是否要删除该信息!")) {
	...
	<%
		...
		<div class="rightinfo">
			...
			<div class="tools">
				...
				<form id="deleteAlls" action="<%=basePath%>organization/toDelete.do"
					method="post">
					<input type="hidden" id="deleteB" name="organization_id" />
				</form>
				<form id="searchAll" action="<%=basePath%>organization/toList.do"
					method="post">
			...
			<tbody>
				...
				<c:if test="${!empty organList}">
					<c:forEach items="${organList}" var="organ">
						<tr>
							<td><input name="select" type="checkbox" value="${organ.organization_id }" />
							</td>
							<td><%=count%></td>
							<td>${organ.name}</td>
							<td>${organ.address}</td>
							<td>${organ.stateVo}</td>
							<td>
								<form id="myFormDelete<%=count%>" action="<%=basePath%>organization/toDelete.do"
									method="post">
									<input type="hidden" name="organization_id" value="${organ.organization_id}" />
								</form>
								<form id="myFormUpdate<%=count%>" action="<%=basePath%>organization/toUpdate.do"
									method="post">
									<input type="hidden" name="organization_id" value="${organ.organization_id}" />
								</form> 
								<a href="javascript:void(0)" onclick="update(<%=count%>)">修改</a>&nbsp;&nbsp; 
								<a href="javascript:void(0)" onclick="deleteOrgan(<%=count++%>)">删除</a>&nbsp;
							</td>
						</tr>
					</c:forEach>
				</c:if>
