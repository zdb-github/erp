...
$(document).ready(function() {
	...
	$(".cancel").click(function() {
		...
		<script type="text/javascript">
function toCodeAdd() {
	window.location.href = "<%=basePath%>code/toCodeAdd.do";
}
function toCodeList() {
	window.location.href = "<%=basePath%>code/toCodeList.do";
}
	...
	<%
		...
		<div class="place">
			...
			<ul class="placeul">
				...
				<li>
					<a href="<%=basePath%>code/toCodeList.do">码表管理</a>
				</li>
				<li>
					<a href="<%=basePath%>code/toCodeList.do">码表列表</a>
				</li>
			</ul>
		</div>
		<div class="rightinfo">
			...
			<div class="tools">
				...
				<form id="deleteAlls" action="<%=basePath%>code/toCodeDelete.do"
					method="post">
					<input type="hidden" id="deleteB" name="id" />
				</form>
				<form id="searchAll" action="<%=basePath%>code/toCodeList.do"
					method="post">
		...
		<div id="green-contents" class="contents">
			...
			<thead  class="header">
				<tr>
					<th width="15%"><input id="selectAll" type="checkbox" onclick="selectAll()" />
					</th>
					<th width="15%">编号</th>
					<th width="15%">键</th>
					<th width="25%">值</th>
			...
			<tbody>
				...
				<c:if test="${!empty codeList }">
					<c:forEach items="${codeList }" var="code">
						<tr>
							<td><input name="select" type="checkbox" value="${code.id}" />
							</td>
							<td><%=count%></td>
							<td>${code.key}</td>
							<td>${code.value}</td>
							<td>${code.type}</td>
							<td>
								<form id="myFormDelete<%=count%>" action="<%=basePath%>code/toCodeDelete.do"
									method="post">
									<input type="hidden" name="id" value="${code.id}" />
								</form>
								<form id="myFormUpdate<%=count%>" action="<%=basePath%>code/toCodeUpdate.do"
									method="post">
									<input type="hidden" name="id" value="${code.id}" />
								</form> 
								<a href="javascript:void(0)" onclick="update(<%=count%>)">修改</a>&nbsp;&nbsp; 
								<a href="javascript:void(0)" onclick="deleteCode(<%=count++%>)">删除</a>&nbsp;
							</td>
						</tr>
					</c:forEach>
				</c:if>
