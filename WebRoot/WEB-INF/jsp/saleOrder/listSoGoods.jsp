	...
	<body>
		...
		<div class="place">
			...
			<ul class="placeul">
				...
				<li>
					<a href="<%=basePath%>saleOrder/toListSoGoods.do">销售订单商品列表</a>
				</li>
		...
		<div class="rightinfo">
			<div class="tools">
				...
				<form id="myFormAdd" action="<%=basePath%>saleOrder/toAddSoGoods.do"
					method="post">
					<input type="hidden" name="so_id" value="${so_id}" />
				</form>
									
				<form id="myFormList" action="<%=basePath%>saleOrder/toListSoGoods.do"
					method="post">
					<input type="hidden" name="so_id" value="${so_id}" />
				</form>
				
				
				<form id="deleteAllGoods" action="<%=basePath%>saleOrder/deleteSoGoods.do"
					method="post">
					<input type="hidden" id="deleteAllGoods" name="id" />
				</form>
				<form id="updateGoods" action="<%=basePath%>saleOrder/toUpdateSoGoods.do"
					method="post">
			...
			<thead  class="header">
				...
				<tbody>
					...
					<c:forEach items="${soGoodsList}" var="gv">
						<tr>
							<td>
								<input name="selectBox" type="checkbox" onclick="selectAll()" value="${gv.id}"/>
							</td>
							<td>
								<%=count%>
							</td>
							...
							<td>
								...
								<td>
									...
									<form id="myFormLook<%=count%>" action="<%=basePath%>saleOrder/toLookSoGoods.do"
										method="post">
										<input type="hidden" name="so_id" value="${gv.so_id}" />
										<input type="hidden" name="id" value="${gv.id}" />
									</form>
									<form id="myFormDelete<%=count%>" action="<%=basePath%>saleOrder/deleteSoGoods.do"
										method="post">
										<input type="hidden" name="id" value="${gv.id}" />
									</form>
									<form id="myFormUpdate<%=count%>" action="<%=basePath%>saleOrder/toUpdateSoGoods.do" method="post">
										<input type="hidden" name="id" value="${gv.id}" />
									</form>
