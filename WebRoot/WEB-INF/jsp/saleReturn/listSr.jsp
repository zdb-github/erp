...
function addInvoices() {
	window.location.href = "<%=basePath%>saleReturn/toAddSr.do";
}
function toListInvoices() {
	window.location.href = "<%=basePath%>saleReturn/toListSr.do";
}
...
<script type="text/javascript">
	...
	<body>
		...
		<div class="place">
			...
			<ul class="placeul">
				...
				<li>
					<a href="<%=basePath%>saleReturn/toListSr.do">销售退货单列表</a>
				</li>
			...
			<!-- 查询采购订单 -->
			<div>
				<form action="<%=basePath%>saleReturn/toListSr.do" method="post">
	...
	<!-- 工具栏 -->
				...
				<form id="deleteAllInvoices"
					action="<%=basePath%>saleReturn/deleteSr.do" method="post">
					<input type="hidden" id="deleteMy" name="sr_id" />
				</form>
				<form id="updateInvoices"
					action="<%=basePath%>saleReturn/toUpdateSr.do" method="post">
					<input type="hidden" id="updateMy" name="sr_id" />
				</form>
				
				<form id="changeStateForm"
					action="<%=basePath%>saleReturn/updateSr.do" method="post">
					<input type="hidden" id="changeMy2" name="sr_id" />
					<input type="hidden" name="invoices_state" value="B"/>
				</form>
				<form id="changeStateForm2"
					action="<%=basePath%>saleReturn/updateSo.do" method="post">
					<input type="hidden" id="changeMy3" name="sr_id" />
		...
		<div id="green-contents" class="contents">
			...
			<thead  class="header">
				...
				<tbody>
					<c:if test="${empty erpSaleReturnList}">
						<tr>
							<td colspan="12">
								里面没有内容
							</td>
					...
					<c:if test="${!empty erpSaleReturnList}">
						<c:forEach items="${erpSaleReturnList}" var="p">
							<tr>
								<td>
									<input name="selectBox" type="checkbox" value="${p.sr_id }" />
								...
								<td>
									<form id="myFormLook<%=count%>"
										action="<%=basePath%>saleReturn/toLookSr.do" method="post">
										<input type="hidden" name="sr_id" value="${p.sr_id}" />
									</form>
									...
									<form id="myFormUpdate<%=count%>"
										action="<%=basePath%>saleReturn/toUpdateSr.do"
										method="post">
										<input type="hidden" name="sr_id" value="${p.sr_id}" />
									</form>
