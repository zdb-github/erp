	...
	$('#updateGoods').click(function () {
                ...
                $( "#alert-win" ).dialog({
				   close: function(event, ui) {
				    ...
				    window.location.href="purchaseOrder/toUpdatePo.do?po_id=${erpPurchaseOrder.po_id}";
				   }
				});
            })
...
<script type="text/javascript">
	function jz(){
		...
		var supplierArr = document.getElementById("supplier_id1");
	  		for(var i=0;i<supplierArr.options.length;i++){
	  			if(supplierArr.options[i].value=="${erpPurchaseOrder.supplier_id}"){
	  		...
	  		for(var i=0;i<deptArr.options.length;i++){
	  			if(deptArr.options[i].value=="${erpPurchaseOrder.dept_id}"){
	  		...
	  		for(var i=0;i<organizationArr.options.length;i++){
	  			if(organizationArr.options[i].value=="${erpPurchaseOrder.organization_id}"){
	  		...
	  		for(var i=0;i<purchaserArr.options.length;i++){
	  			if(purchaserArr.options[i].value=="${erpPurchaseOrder.purchaser_id}"){
	  		...
	  		for(var i=0;i<warehouseArr.options.length;i++){
	  			if(warehouseArr.options[i].value=="${erpPurchaseOrder.warehouse_id}"){
	  		...
	  		for(var i=0;i<stateArr.options.length;i++){
	  			if(stateArr.options[i].value=="${erpPurchaseOrder.invoices_state}"){
	...
	<% int count =1; %>
		<div class="alert alert-info">
			当前位置
			<b class="tip"></b>采购管理
			<b class="tip"></b>修改采购订单
		</div>
		<div id="po">
		<form action="purchaseOrder/updatePo.do" method="post" id="myForm1"> 
			<table class="table table-striped table-bordered table-condensed">
				<thead>
					<tr>
						<td colspan="6" class="auto-style2">
							&nbsp;请修改采购订单${erpPurchaseOrder.po_id}  的主表内容
						</td>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<td >
							<input id="po_id1" name="po_id" style="width:220px; height:30px"  type="text" value="${erpPurchaseOrder.po_id}" readonly="readonly" />
						</td>
						<td>
							单据日期
						</td>
						<td>
							<input id="date1" class="span2 datepicker" style="width:220px; height:30px" type="text" name="create_time" value="${erpPurchaseOrder.create_time}" />
	...
	<div id="alert-win" title="修改采购订单商品" style="display: none;">
        <iframe style="border: 0; width: 100%; height: 100%;" src="purchaseOrder/toListPoGoods.do?po_id=${erpPurchaseOrder.po_id}"></iframe>
    </div>
